...
- [x] 1.1 Replace `asyncpg` with `aiomysql` in `pyproject.toml` (`uv add aiomysql && uv remove asyncpg`).
- [x] 1.2 Update default `DATABASE_URL` in `src/infra/config.py` to `mysql+aiomysql://shorty:shorty@localhost:3306/shorty`.
- [x] 1.3 Update default `sqlalchemy.url` in `alembic.ini` to match.
